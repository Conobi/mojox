...
class TestPurityBoundary:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.Import):
                for alias in node.names:
                    assert alias.name != "subprocess", f"{module_name} imports subprocess"
            elif isinstance(node, ast.ImportFrom) and node.module and "subprocess" in node.module:
    ...
    def test_no_os_environ_access(self, module_name):
        ...
        assert source_file is not None
        source = Path(source_file).read_text()
        assert "os.environ" not in source, f"{module_name} accesses os.environ"
